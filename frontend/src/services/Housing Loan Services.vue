<template>

  <!-- Banner Section -->
  <section class="px-4 sm:px-6 lg:px-10 my-8">

    <div
      class="relative w-full h-[300px] sm:h-[500px] lg:h-[650px] overflow-hidden rounded-[40px]"
    >

      <!-- Banner Image -->
      <img
        src="/files/homeloanbanner.png"
        alt="Housing Loan Services"
        class="w-full h-full object-cover"
      />

      <!-- Overlay -->
      <div class="absolute inset-0 bg-black/45 rounded-[40px]"></div>

      <!-- Content -->
      <div
        class="absolute inset-0 flex flex-col items-center justify-center text-center px-6"
      >

        <h1
          class="text-white text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight"
        >
          Housing Loan <br />
          Services
        </h1>

        <p
          class="text-white/90 text-sm sm:text-lg lg:text-xl mt-5 max-w-3xl leading-8"
        >
          Making your dream home affordable with flexible and reliable
          housing loan solutions tailored to your financial needs.
        </p>

      </div>

    </div>

  </section>


  <!-- About Section -->
  <section class="py-16 px-4 sm:px-6 lg:px-10">

    <div
      class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14 items-center"
    >

      <!-- Left -->
      <div>

        <p class="text-[#4b4b9b] font-semibold text-lg mb-4">
          Trusted Financial Guidance
        </p>

        <h2
          class="text-3xl sm:text-3xl lg:text-3xl font-bold text-[#2f2f68] leading-tight mb-8"
        >
          Simplifying Home Loan Solutions For You
        </h2>

        <div class="space-y-6">

          <p class="text-gray-600 text-base sm:text-lg leading-8">
            Our Housing Loan Services help individuals and families
            secure the best financial support for purchasing their
            dream homes. We work closely with leading banks and
            financial institutions to provide competitive interest
            rates and flexible repayment options.
          </p>

          <p class="text-gray-600 text-base sm:text-lg leading-8">
            From loan eligibility assessment to document processing
            and final approval, our team guides you through every
            stage of the housing loan journey with complete
            transparency and support.
          </p>

          <p class="text-gray-600 text-base sm:text-lg leading-8">
            Whether you are buying your first home, investing in
            property, or planning construction financing, we ensure
            a smooth and hassle-free loan experience.
          </p>

        </div>

      </div>

      <!-- Right -->
      <div>

        <img
          src="/files/homeloanabout.png"
          alt="Housing Loan"
          class="w-full h-[450px] rounded-[30px] shadow-xl object-cover"
        />

      </div>

    </div>

  </section>


  <!-- Services Section -->
  <section class="bg-[#f8f8f8] py-16 px-4 sm:px-6 lg:px-10">

    <div class="max-w-7xl mx-auto">

      <!-- Heading -->
      <div class="text-center mb-14">

        <p class="text-[#4b4b9b] font-semibold text-lg">
          Our Loan Services
        </p>

        <h2
          class="text-3xl sm:text-3xl lg:text-3xl font-bold text-[#2f2f68] mt-3"
        >
          What We Provide
        </h2>

      </div>

      <!-- Cards -->
      <div
        class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8"
      >

        <!-- Card -->
        <div
          class="bg-white rounded-3xl p-8 shadow-lg hover:-translate-y-2 transition duration-300"
        >
          <div class="text-5xl mb-5">🏦</div>

          <h3 class="text-2xl font-semibold text-[#2f2f68] mb-4">
            Home Loan Assistance
          </h3>

          <p class="text-gray-600 leading-8 text-base">
            Get support in choosing the best housing loan plans
            with attractive interest rates and repayment options.
          </p>
        </div>

        <!-- Card -->
        <div
          class="bg-white rounded-3xl p-8 shadow-lg hover:-translate-y-2 transition duration-300"
        >
          <div class="text-5xl mb-5">📄</div>

          <h3 class="text-2xl font-semibold text-[#2f2f68] mb-4">
            Documentation Support
          </h3>

          <p class="text-gray-600 leading-8 text-base">
            Complete assistance with loan paperwork, verification,
            and submission processes for faster approvals.
          </p>
        </div>

        <!-- Card -->
        <div
          class="bg-white rounded-3xl p-8 shadow-lg hover:-translate-y-2 transition duration-300"
        >
          <div class="text-5xl mb-5">💳</div>

          <h3 class="text-2xl font-semibold text-[#2f2f68] mb-4">
            EMI Planning
          </h3>

          <p class="text-gray-600 leading-8 text-base">
            Expert financial guidance to help you manage monthly
            EMIs comfortably within your budget.
          </p>
        </div>

      </div>

    </div>

  </section>


  <!-- Benefits Section -->
  <section class="pt-16 pb-0 px-4 sm:px-6 lg:px-10">

    <div class="max-w-6xl mx-auto text-center">

      <p class="text-[#4b4b9b] font-semibold text-lg mb-4">
        Why Choose Us
      </p>

      <h2
        class="text-3xl sm:text-3xl lg:text-3xl font-bold text-[#2f2f68] mb-10"
      >
        Benefits Of Our Housing Loan Services
      </h2>

      <div
        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8"
      >

        <div class="bg-white shadow-lg rounded-3xl p-8">
          <h3 class="text-2xl font-semibold text-[#2f2f68] mb-3">
            Low Interest Rates
          </h3>

          <p class="text-gray-600 leading-8">
            Access competitive loan interest rates from trusted banks.
          </p>
        </div>

        <div class="bg-white shadow-lg rounded-3xl p-8">
          <h3 class="text-2xl font-semibold text-[#2f2f68] mb-3">
            Fast Approval
          </h3>

          <p class="text-gray-600 leading-8">
            Quick processing and smooth loan approval assistance.
          </p>
        </div>

        <div class="bg-white shadow-lg rounded-3xl p-8">
          <h3 class="text-2xl font-semibold text-[#2f2f68] mb-3">
            Trusted Guidance
          </h3>

          <p class="text-gray-600 leading-8">
            Professional support throughout your loan process.
          </p>
        </div>

        <div class="bg-white shadow-lg rounded-3xl p-8">
          <h3 class="text-2xl font-semibold text-[#2f2f68] mb-3">
            Flexible EMI Options
          </h3>

          <p class="text-gray-600 leading-8">
            Convenient repayment plans designed for your comfort.
          </p>
        </div>

      </div>

    </div>
    <!-- CTA Section -->
<section
  class="relative left-1/2 right-1/2 -mx-[50vw] mt-20 w-screen bg-[#0B1560] pt-16 pb-8 mb-0 "
>

  <div class="text-center px-5">

    <h1
      class="text-white text-[32px] sm:text-[45px] font-bold"
    >
      Ready To Build Your Dream Project?
    </h1>

    <p
      class="text-white/80 text-[15px] leading-8 mt-5 max-w-2xl mx-auto"
    >
      Contact Per Square Feet today for premium
      construction services and trusted project execution.
    </p>

    <button
      class="mt-8 bg-[#d49a37c9] hover:bg-[#d49a37] text-white px-8 py-4 rounded-2xl text-[16px] font-semibold transition duration-300"
    >
      Contact Us
    </button>

  </div>

</section>
  </section>

</template>